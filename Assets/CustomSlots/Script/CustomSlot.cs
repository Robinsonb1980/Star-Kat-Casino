...
namespace CSFramework {
	...
	public class CustomSlot : MonoBehaviour {
		...
		public void Activate() {
			...
			for (int i = reels[0].holders.Count - 1; i >= 0; i--) foreach (var r in reels) r.ApplyMultiRow(i);
		...
		public Sequence DisplayHitEffect(HitInfo info) {
			...
			sequence.OnStart(() => { callbacks.onProcessHit.Invoke(info); });
		...
		private void StartPlaybackResult() {
			...
			foreach (HitInfo hitInfo in gameInfo.scatterHitInfos) if (!hitInfo.hitSymbol.ignoreThisRound && hitInfo.isHit) sequenceResult.Append(DisplayHitEffect(hitInfo));
		}

		public void StopPlaybackResult() {
			if (sequenceResult != null) {
				sequenceResult.Complete(false);
				sequenceResult = null;
			}
		}
