	...
	public class SymbolGen {
		...
		private void MakeNewReels() {
			...
			foreach (Symbol symbol in symbolManager.symbols)
				if (symbol.minCountPerReel > 0 && !symbol.isMRS)
			...
			for (int x = 0; x < reelLength; x++) {
				foreach (Symbol symbol in symbolManager.symbols) {
					if (!symbol.isMRS) continue;

					// a is the number of the multi-row-symbol(MRS) you want to spawn
					// You can implement your own logic here.
					// Like if you always want to spawn 5 MRS, simply set a=5;
					int a = Random.Range(1, 3);

					// maxTry represents how many times you want to try placing the MRS.
					// if it reaches 0, it won't place the MRS any more.
					var maxTry = 100;

					while (maxTry > 0 && a > 0) {
						int y = Random.Range(lastIndex, symbolsPerReel - symbol.rowSize - 2);
						bool canPlace = true;
						for (int y2 = y; y2 < y + symbol.rowSize + 2; y2++) if (reels[x][y2].isMRS) canPlace = false;
						if (canPlace) {
							for (int y2 = y + 1; y2 < y + symbol.rowSize + 1; y2++) reels[x][y2] = symbol;
							a--;
						}
						maxTry--;
					}
